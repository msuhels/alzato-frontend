...
const UpdatesPage = () => {
  ...
  // Filter items to show only last 24 hours
  const last24HoursItems = useMemo(() => {
    const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
    return items.filter(item => new Date(item.created_at) >= twentyFourHoursAgo);
  }, [items]);
  ...
  return (
    <div>
      ...
      <div className="rounded-lg bg-white p-6 shadow-sm">
        ...
        ) : last24HoursItems.length === 0 ? (
          <div className="text-center text-gray-custom-500 py-8">No updates from the last 24 hours.</div>
        ) : (
